...
const ensureReportColumns = async (connection) => {
  ...
  if (!colNames.includes("post_id")) {
    await connection.query(
      "ALTER TABLE reports ADD COLUMN post_id INT DEFAULT NULL",
    );
    await connection.query(
      "ALTER TABLE reports ADD FOREIGN KEY (post_id) REFERENCES posts(post_id) ON DELETE CASCADE",
    );
    await connection.query(
      "ALTER TABLE reports ADD INDEX idx_reports_post_id (post_id)",
    );
  }

  if (!colNames.includes("comment_id")) {
    await connection.query(
      "ALTER TABLE reports ADD COLUMN comment_id INT DEFAULT NULL",
    );
    await connection.query(
      "ALTER TABLE reports ADD FOREIGN KEY (comment_id) REFERENCES comments(id) ON DELETE CASCADE",
    );
    await connection.query(
      "ALTER TABLE reports ADD INDEX idx_reports_comment_id (comment_id)",
    );
  }
};
